    ...
    def __call__(self, url):
        try:
            ...
            if box is not None:
                ...
                box_data = box.find("div", attrs=
                {"class":"contentSectionDetails"})
                self.news["date"] = (
                    datetime.strptime(
                    "".join(self.date_pattern.findall(box_data.find("span").text)), "%b %d, %Y %H:%M%p"
                    )
                    .replace(tzinfo=timezone.utc)
                    .timestamp()
                )
                article = box.find("div", attrs={"class": "articlePage"})
                text_block = article.find_all("p")
                self.news["text"] = "\n".join([p.text for p in text_block]).strip()
